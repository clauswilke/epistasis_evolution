...
in_dir <- paste0(root_dir, "/evolving_q_sim/test_run/k_distributions/") # set the directory that contains the simulation results to be concatenated
outfile <- paste0(root_dir, "/evolving_q_sim/processed_results/evolved_q_k_distr.csv") # name the CSV output file

# get a list of files to be concatenated from the directory `in_dir`
f_lst <- list.files(path = in_dir, pattern = "rep\\d+_k_distr.csv", full.names = T)
