...
if [ ! -d equilib_test/N${eff_pop}_max${max_move} ]; then
	mkdir equilib_test/N${eff_pop}_max${max_move}
fi
...
do
	...
	do 	
		...
		do
			...
			do
				outfile=complexity_evolution/bitstring_model/equilib_test/N${eff_pop}_max${max_move}/eps${eps}_s${sel_coef}_m${mut_prob}_k_start${k}.txt
