...
do
	...
	do 	
		...
		do
			...
			do
				out_file=complexity_evolution/bitstring_model/varying_eps_sim/eps${eps}_s${sel_coef}_m${mut_prob}_rep${i}.txt
				echo python complexity_evolution/bitstring_model/src/evolve.py -s $sel_coef -m $mut_prob -N $eff_pop -L $num_class -k_start $k_start -eps $eps -o $out_file >> src/run_simulation.sh 
			done
		done
	done
done
